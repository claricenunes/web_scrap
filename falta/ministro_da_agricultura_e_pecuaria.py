...
def extract_minister(soup, base_url=URL):
    ...
    # name
    name = "Carlos Fávaro"
    if "Carlos Fávaro" in text:
        name = "Carlos Fávaro"
    
    # title
    title = "Ministro de Estado da Agricultura e Pecuária"
    
    # emails
    emails = []
    email_match = re.search(r'[\w\.-]+@agro\.gov\.br', text)
    if email_match:
        emails.append(email_match.group(0).strip())
    
    # phones
    phones = []
    phone_match = re.search(r'Telefone:\s*(\(61\)\s*[\d\s\-/]+)', text)
    if phone_match:
...
def main():
    ...
    out_file_base = "ministro_da_agricultura_e_pecuaria"
